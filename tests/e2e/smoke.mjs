...
try {
  ...
  check("manifest version_name = 5.2.26", manifest.version_name === "5.2.26", `actual: ${manifest.version_name}`);
  // v5.2.19+: 正式改名 AI圆桌派（v5.2.26 仍保持）
  ...
  check("sidepanel version badge", versionBadge === "v5.2.26", `actual: "${versionBadge}"`);

  const footerVersion = await sidepanelPage.locator(".footer").textContent();
  check("sidepanel footer version", footerVersion?.includes("v5.2.26"), `actual: "${footerVersion?.slice(0, 100)}"`);
  ...
  check("popup chat-version = v5.2.26", popupVersion === "v5.2.26", `actual: "${popupVersion}"`);
  ...
  check("v4.7.2: background exportSession 返回 markdown",
    exportFixed.ok && exportFixed.hasMarkdown && exportFixed.markdownStart.includes("AI圆桌派"),
  ...
  // ── v5.2.2-beta polish: placeholder 动态化 + 重置弹窗美化 ──
  ...
  check("v5.2.2-beta polish ①: popup-task-menu 含 PLACEHOLDER_BY_TASK 4 模式 + updatePlaceholder",
  ...
  check("v5.2.2-beta polish ① 运行时: setTask('debate') 后 placeholder 含'辩论引导'+'留空'",
  ...
  check("v5.2.2-beta polish ① 运行时: 切回 ask 后 placeholder 恢复原文",
    placeholderRuntime.afterAsk.includes("Ctrl+Enter 发送") && placeholderRuntime.afterAsk.includes("@ 单发"),
    `actual: ${JSON.stringify(placeholderRuntime)}`);

  // v5.2.2-beta fix: ChatModal escListener 不再泄漏（点按钮关 modal 后 document 上无残留 keydown）
  ...
  check("v5.2.2-beta fix: 点按钮关 modal 后 Enter 不再泄漏触发 primary.onClick（修彻底重置 bug）",
  ...
  const v52ApiRuntime = await popupPage.evaluate(() => ({
    ...
    hasNewerSelfTest: window.ChatUpdateCheck?._hasNewer?.("5.2.26", "v5.3.0-beta"),
    hasNewerSameTest: window.ChatUpdateCheck?._hasNewer?.("5.2.26", "v5.2.26"),
  }));
  check("v5.2.0 运行时: ChatUpdateCheck API 暴露 + currentVersion 返回 5.2.26 + hasNewer 比对逻辑正确",
    v52ApiRuntime.hasApi && v52ApiRuntime.hasCurrentVersion && v52ApiRuntime.hasNewerHelper &&
    v52ApiRuntime.curVer === "5.2.26" &&
    v52ApiRuntime.hasNewerSelfTest === true &&
    v52ApiRuntime.hasNewerSameTest === false,
    JSON.stringify(v52ApiRuntime));

  // ── v5.2.2: 豆包 selector 修正（v5.2.1 推断 bg-g-receive 错了，用户登录态实测真命名是 v_list_row） ──
  const selectorsV521 = fs.readFileSync(path.join(EXT_PATH, "selectors-config.js"), "utf8");
  check("v5.2.2: doubao response 用 v_list_row:not(:has([bg-g-send]))（用户实测命中）",
    /\[class\*="v_list_row"\]:not\(:has\(\[class\*="bg-g-send"\]\)\)/.test(selectorsV521),
    "doubao response 缺 v_list_row 主 selector");
  check("v5.2.2: doubao input 含 textarea.semi-input-textarea（Semi Design）",
    /textarea\.semi-input-textarea/.test(selectorsV521),
    "doubao input selector 缺 Semi Design 命名");
  check("v5.2.2: doubao userMessage 含 bg-g-send + send-msg-bubble-text",
  ...
  // ── v5.2.9: hardReset 后 ChatTaskMenu task 拉回 ask（修"重置后再加 AI 无法群聊发送"沉默 fail）──
  const taskMenuSrc = fs.readFileSync(path.join(EXT_PATH, "popup-task-menu.js"), "utf8");
  check("v5.2.9: popup-task-menu.js 监听 hardReset 消息",
    /chrome\.runtime\.onMessage\.addListener/.test(taskMenuSrc) &&
    /msg\?\.type\s*===\s*["']hardReset["']/.test(taskMenuSrc),
    "popup-task-menu.js 缺 hardReset 监听 — 重置后 task 不会拉回 ask");
  ...
  // ── v5.2.15: 豆包/Kimi/元宝 sendButton 防御加强（aria-label 兜底 + 豆包精准 send-msg-btn）──
  check("v5.2.15: 豆包 sendButton 加 send-msg-btn 精准 selector（MCP 实测豆包真 class）",
    /'button\[class\*="send-msg-btn"\]'/.test(selSrc),
    "豆包 sendButton 缺 send-msg-btn 精准");
  check("v5.2.15: Kimi sendButton 加 aria-label 兜底",
    /kimi:[\s\S]{0,2000}?sendButton[\s\S]*?'button\[aria-label\*="发送"\]'/.test(selSrc),
    "Kimi sendButton 缺 aria-label 防御");
  check("v5.2.15: 元宝 sendButton 加 aria-label 兜底",
    /yuanbao:[\s\S]{0,2000}?sendButton[\s\S]*?'button\[aria-label\*="发送"\]'/.test(selSrc),
    "元宝 sendButton 缺 aria-label 防御");
